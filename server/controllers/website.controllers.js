...
export const changes = async (req, res) => {
  try {
    ...
    if (user.credits < 25) {
      return res.status(400).json({ message: "You do not have enough credits to update the website" });
    }
...
`;
    ...
    // console.log(error)
    return res.status(500).json({ message: `Update website error: ${error.message}` });
  }
};
