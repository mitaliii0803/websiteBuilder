...
import { ArrowLeft, Code, Code2, Monitor, Rocket, Send, X , MessageSquare} from 'lucide-react'
...
function WebsiteEditor () {
  ...
  const handleUpdate=async () => {
    ...
    try {
      const result = await axios.post(`${serverUrl}/api/website/update/${id}`,{prompt:text},
...
//fetch website data by ID
  ...
  return (
    ...
    <input placeholder='Describe Changes... ' className='flex-1
    resize-none rounded-2xl px-4 py-3 bg-white/5 border border-white/10 text-sm
    outline-none' onChange={(e)=>setPrompt(e.target.value)} value={prompt}/>
     <button className='px-4 py-3 rounded-2xl bg-white text-black' disabled={updateLoading}
...
<div className='flex-1 flex flex-col'>
  <div className='h-14 px-4 flex justify-between items-center border-white/10 bg-black/80'>
    ...
    to-purple-500 text-sm font-semibold hover:scale-105 transition'
    ...
    <button className='p-2' onClick={()=> setShowFullPreview(true)}><Monitor 
    size={18}/></button>
    </div>
...
> 
     ...
     border-b border-white/10 bg-[#1e1e1e]">
