...
function Home() {
  ...
  <div className="hidden md:flex items-center gap-2 px-3 py-1.5
   rounded-full bg-white/5 border border-white/10 text-sm 
  ...
  <button className='flex items-center' onClick={()=>setOpenProfile(!openProfile)}>
    <img src={userData.avatar || `https://ui-avatars.com/api/?name=${userData.name}`} 
        alt="abc" referrerPolicy='no-referrer'className='w-9 h-9 rounded-full border border-white/20 object-cover'/>
</button>
<AnimatePresence>
  {openProfile && (
    <>
...
>
  ...
  <div className='px-4 py-3 border-b border-white/10'>
    ...
    <button className='md:hidden w-full px-4 py-3 flex 
    ...
    <button className='w-full px-4 py-3 text-left text-sm text-red-400
     hover:bg-white/5' onClick={handleLogOut}>Logout</button>
</motion.div>
    </>
  )
  }
    ...
    <section className='pt-44 pb-32 px-6 text-center'>
      ...
      transition mt-12"  onClick={()=> userData? navigate("/dashboard"):setOpenLogin(true)}> 
    ...
    <section className='max-w-7xl mx-auto px-6 pb-32'>
      <div className='grid grid-cols-1 md:grid-cols-3 gap-10'>
        {highlights.map((h,i)=>(     //highlight-h and index-i
          <motion.div
          key={i}
          initial={{opacity:0, y:40}}
